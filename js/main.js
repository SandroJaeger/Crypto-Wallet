...
if ('serviceWorker' in navigator) {
	navigator.serviceWorker.register('/Crypto-Wallet/sw.js');
}
		...
		[...document.getElementsByClassName("switch")].forEach(sw => {
			sw.addEventListener("click", function(){
				...
				if($(this).attr('class').includes("active")){
					...
					firebaseRef = firebase.database().ref("Users/"+accountID);
					firebaseRef.child("connection").set("disconnected");
				}
			});
		});
		firebaseRef = firebase.database().ref("Users/"+accountID);
...
document.getElementById("sell-popup-btn").addEventListener("click", function( event ) {
	...
	if(document.getElementById("input-amount").value != "" && document.getElementById("input-address").value != "" && document.getElementById("input-amount").value <= total){
		...
		showtoast("warning", "The amount or address is wrong!");
	}
	...
	for (var i = 0; i < creditCards.length; i++) {
		if (creditCards[i] === card) {
			for (var n = 0; n < cards[i].transactions.length; n++) {
				...
				output += "</div>";
				output += "<div class='transaction-item_amount'><span><svg width='11' height='18' xmlns='http://www.w3.org/2000/svg'><path d='M11 10.216 5.5 18 0 10.216l5.5 3.263 5.5-3.262ZM5.5 0l5.496 9.169L5.5 12.43 0 9.17 5.5 0Z' fill='#00FFF8'></path></svg>&nbsp;&nbsp;</span><p class='amount'>"+ cards[i].transactions[n].amount + "&nbsp;ETH" +"</p></div>";
				output += "</div>";
